    ...
    private async fetchData() {
        ...
        for (const result of results) {
            if (result.status === 'fulfilled') {
                ...
                console.error(
                    `Error fetching ${result.reason.key}:`,
                    result.reason.error,
                );
    ...
    public async _getCompetitors(
        ...
        try {
            ...
            if (
                ...
                competitors.every((item) => typeof item === 'string')
            ) {
                return competitors;
            } else {
                throw new Error(
                    'Invalid response format: Expected an array of strings',
                );
            }
        } catch (error) {
...
function getTickers() {
    const scraper = new TickerScraper(
